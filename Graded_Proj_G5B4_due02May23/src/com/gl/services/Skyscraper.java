...
public class Skyscraper {
	...
	public static void calculateFloors(int[] floorSize) {
		...
		for (int i = 0; i < floorSize.length; i++) {
			...
			/*
			 * if (size > maxSoFar) { for (int j = maxSoFar + 1; j <= size; j++) {
			 * floorSizes.push(j); } maxSoFar = size; }
			 * 
			 * if (size == floorSizes.peek()) { floorSizes.pop(); System.out.print(size); if
			 * (!floorSizes.empty()) { System.out.print(" "); }
			 */
